                    ...
                    <form class="pt-3" id="create-form" action="{{ route('class-group.store') }}" method="POST" novalidate="novalidate" enctype="multipart/form-data">
                        <div class="row">
                            ...
                            <div class="form-group col-sm-12 col-md-6">
                                ...
                                <select name="class_ids[]" class="form-control select2-dropdown select2-hidden-accessible" multiple required>
                                    @foreach($classes as $value => $label)
                                    <option value="{{ $value }}" {{ collect(old('class_ids'))->contains($value) ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <div class="modal-content">
                    ...
                    <form class="pt-3 subject-edit-form" id="edit-form" action="{{ url('class-group') }}"
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label>{{ __('Classes') }} <span class="text-danger">*</span></label>
                                <select name="class_ids[]" id="edit_class_ids" class="form-control select2-dropdown select2-hidden-accessible" multiple required>
                                    @foreach($classes as $value => $label)
                                    <option value="{{ $value }}" {{ collect(old('class_ids'))->contains($value) ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
